...
MOSAMMAT JOYAYRIYA RAHMAN, 21-44445-1
ABU SYED MOHAMMED SAFWAN, 21-44496-1
UMME JANNATUL FARIHA, 21-45718-3
ASIR FOYSAL AL MUKIT, 21-45696-3
*/
...
GLfloat position = 0.0; //variable,train
GLfloat speed = 0.8;
GLfloat position1 = 0.0; //sun
GLfloat position2 = 0.0; //cloud
GLfloat position4 = 0.0;
GLfloat position5 = 0.0;
GLfloat position6 = 0.0;

GLfloat position7 = 0.0;//sun
...
void sun(int value) //
{
    if(position1 > 20.0)
        position1 = -20.0;
        position1 += speed1;
        glutPostRedisplay(); //new jeta add seta show
        glutTimerFunc(100, sun, 1);
}
void train(int value)
{
    if(position > 10.0) // screen theke ber hye jsse,then abr first theke start hbe
        position = -20.0;
        position += speed; //exact tym por agai
        glutPostRedisplay();
...
void sun(bool sunny) {
    ...
    // Conditions for drawing the sun
    if (!night && !rainday) {
        // Push the current matrix onto the matrix stack
        glPushMatrix();

        // Translate to the position where the sun will be drawn
        glTranslatef(7.0f, 15.0f, 0.0f);

        // Set up parameters for drawing a circle
        GLfloat radius = 2.0f;
        GLfloat twicePi = 2.0f * PI;

        // Begin drawing a filled circle using triangle fan
        glBegin(GL_TRIANGLE_FAN);

        // Set the color of the sun
        glColor3f(1.0f, 0.0f, 0.2f); //center color
        ...
        // Iterate through segments to draw the circle
        for (int i = 0; i <= num_segments; i++) {
            GLfloat theta = i * twicePi / num_segments;
            GLfloat x = radius * cos(theta);
            GLfloat y = radius * sin(theta);

            // Vary the color based on the position in the circle
            glColor3f(1.0f, 0.8f - 0.6f * y / radius, 0.2f - 0.2f * y / radius); //lower part color
...
void init(void) //initializing func
{
    // Set the clear color (background color) to a light blue shade
    glClearColor(0.77, 0.95, 1.00, 1.0);

    // Set the matrix mode to projection matrix
    glMatrixMode(GL_PROJECTION);// 3D scene is projected onto a 2D plane.
    ...
    //glOrtho: Defines an orthographic projection matrix. It is used to specify a 2D orthographic projection where objects are the same size regardless of their depth. The parameters define the clipping planes for left, right, bottom, top, near, and far. In this case, it sets up an orthographic projection with the left edge at 0, right edge at 32, bottom at 0, top at 18, near clipping plane at -10, and far clipping plane at 10
...
int main(int argc, char** argv)
{
    ...
    cout << endl << "Mass rapid transit system Dhaka Metro Rail" << endl << endl;
